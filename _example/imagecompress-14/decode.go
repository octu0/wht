...
func decodeBase(r io.Reader) (*image.YCbCr, error) {
	...
	for _, s := range subbands {
		if nextBufs, err := decodeSubband(yBufs, currentImg.Y, w, h, s.sx, s.sy, s.ex, s.ey); err != nil {
			return nil, errors.WithStack(err)
		} else {
			yBufs = nextBufs
		}
	...
	for _, s := range cSubbands {
		if nextBufs, err := decodeSubband(cbBufs, currentImg.Cb, cw, ch, s.sx, s.sy, s.ex, s.ey); err != nil {
			return nil, errors.WithStack(err)
		} else {
			cbBufs = nextBufs
		}
	...
	// Cr
	for _, s := range cSubbands {
		if nextBufs, err := decodeSubband(crBufs, currentImg.Cr, cw, ch, s.sx, s.sy, s.ex, s.ey); err != nil {
			return nil, errors.WithStack(err)
		} else {
			crBufs = nextBufs
		}
