...
const (
	...
	mb24x24_8  macroblock = 7
)

// macroblockPartition はマクロブロックの分割情報を保持します
...
// CalcMacroblockY はYプレーンのマクロブロックタイプを決定します
func (r *ImageReader) CalcMacroblockY(w, h int) macroblock {
	if similarblock(w, h, 32, r.headtailY) {
		return mb32x32
	}
	if similarblock(w, h, 24, r.headtailY) {
		return mb24x24_8
	}
...
// CalcMacroblockCb はCbプレーンのマクロブロックタイプを決定します
...
// CalcMacroblockCr はCrプレーンのマクロブロックタイプを決定します
