...
func main() {
	...
	headtailCb := func(w, h int, n int) (uint8, uint8) {
		i1 := ycbcr.COffset(w*2, h*2)
		i2 := ycbcr.COffset((w+n)*2, h*2)
		if len(ycbcr.Cb) <= i1 {
			return 0, 0
		}
		if len(ycbcr.Cb) <= i2 {
			return ycbcr.Cb[i1], 0
		}
		return ycbcr.Cb[i1], ycbcr.Cb[i2]
	}
	headtailCr := func(w, h int, n int) (uint8, uint8) {
		...
		if len(ycbcr.Cr) <= i2 {
			return ycbcr.Cr[i1], 0
		}
		return ycbcr.Cr[i1], ycbcr.Cr[i2]
	}
	...
	rowCb := func(x, y int, size int, prediction int16) []int16 {
		plane := make([]int16, size)
		width := ycbcr.Rect.Dx()
		height := ycbcr.Rect.Dy()
		for i := 0; i < size; i += 1 {
	...
	rowCr := func(x, y int, size int, prediction int16) []int16 {
		plane := make([]int16, size)
		width := ycbcr.Rect.Dx()
		height := ycbcr.Rect.Dy()
		for i := 0; i < size; i += 1 {
	...
	tmpImg := image.NewYCbCr(ycbcr.Bounds(), image.YCbCrSubsampleRatio420)
	newImg := image.NewYCbCr(ycbcr.Bounds(), image.YCbCrSubsampleRatio420)
	type setRowFunc func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16)
	setRowY := func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16) {
		width := img.Rect.Dx()
		height := img.Rect.Dy()
		for i := 0; i < size; i += 1 {
			...
			img.Y[img.YOffset(x+i, y)] = clampU8(plane[i] + prediction)
		}
	}
	setRowCb := func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16) {
		width := img.Rect.Dx()
		height := img.Rect.Dy()
		for i := 0; i < size; i += 1 {
			px := (x + i) * 2
			...
			img.Cb[img.COffset(px, py)] = clampU8(plane[i] + prediction)
		}
	}
	setRowCr := func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16) {
		width := img.Rect.Dx()
		height := img.Rect.Dy()
		for i := 0; i < size; i += 1 {
			px := (x + i) * 2
			...
			img.Cr[img.COffset(px, py)] = clampU8(plane[i] + prediction)
		}
	}
	...
	acQuantize := func(data []int16, quantizeTable QuantizationTable, scale int) []int8 {
		if scale < 1 {
			scale = 1
		}
	...
	acDequantize := func(data []int8, quantizeTable QuantizationTable, scale int) []int16 {
		if scale < 1 {
			scale = 1
		}
	...
	writeIndex := func(out io.Writer, idx uint16) error {
		if idx < 128 {
			return binary.Write(out, binary.BigEndian, uint8(idx))
		}
		return binary.Write(out, binary.BigEndian, uint16(idx)|0x8000)
	}
	readIndex := func(in io.Reader) (uint16, error) {
		b := uint8(0)
		...
		b2 := uint8(0)
		if err := binary.Read(in, binary.BigEndian, &b2); err != nil {
			return 0, errors.WithStack(err)
		}
		return (uint16(b&0x7f) << 8) | uint16(b2), nil
	}
	...
	dcEncode := func(out io.Writer, dc []int16, size int) error {
		...
		for i := 0; i < size; i += 2 {
			...
			if err := writeIndex(out, dictIndex); err != nil {
				return errors.WithStack(err)
			}
		}
		return nil
	...
	transform := func(out io.Writer, dcQT, acQT QuantizationTable, data [][]int16, size, scale int) error {
		// write scale
	...
	transformHandlerFunc := func(w, h int, size int, dcQT, acQT QuantizationTable, rowN rowFunc, setRow setRowFunc, predict predictDCFunc, scale int) (*bytes.Buffer, error) {
		...
		for i := 0; i < size; i += 1 {
			...
			setRow(tmpImg, w, h+i, size, planes[i], prediction)
		}
		return data, nil
	}

	invertHandlerFunc := func(in io.Reader, w, h int, size int, dcQT, acQT QuantizationTable, setRow setRowFunc, predict predictDCFunc) error {
		prediction := predict(newImg, w, h, size)
		...
		for i := 0; i < size; i += 1 {
			setRow(newImg, w, h+i, size, planes[i], prediction)
		}
		return nil
	}
	...
	calcScale := func(addedBits, addedPixels int) int {
		currentBits += addedBits
		processedPixels += addedPixels

		// target bits for current progress
		targetBits := float64(maxbit) * (float64(processedPixels) / float64(totalProcessPixels))
		diff := float64(currentBits) - targetBits
	...
	for h := 0; h < dy; h += 32 {
		for w := 0; w < dx; w += 32 {
			...
			for _, p := range parts {
				...
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, lumaQT, rowY, setRowY, predictYPlaneDC, scale)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
				bufY = append(bufY, data)
				scale = calcScale(data.Len()*8, p.Size*p.Size)
			}
	...
	for h := 0; h < dy/2; h += 16 {
		for w := 0; w < dx/2; w += 16 {
			...
			for _, p := range parts {
				...
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, chromaQT, rowCb, setRowCb, predictCbPlaneDC, scale)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
	...
	for h := 0; h < dy/2; h += 16 {
		for w := 0; w < dx/2; w += 16 {
			...
			for _, p := range parts {
				...
				qtPair := qtBySize(p.Size)
				dcQT, chromaQT := qtPair[0], qtPair[2]
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, chromaQT, rowCr, setRowCr, predictCrPlaneDC, scale)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
	...
	for h := 0; h < dy; h += 32 {
		for w := 0; w < dx; w += 32 {
			...
			for _, p := range parts {
				...
				qtPair := qtBySize(p.Size)
				dcQT, lumaQT := qtPair[0], qtPair[1]
				if err := invertHandlerFunc(in, blockX, blockY, p.Size, dcQT, lumaQT, setRowY, predictYPlaneDC); err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
			}
		}
	...
	for h := 0; h < dy/2; h += 16 {
		for w := 0; w < dx/2; w += 16 {
			...
			for _, p := range parts {
				...
				qtPair := qtBySize(p.Size)
				dcQT, chromaQT := qtPair[0], qtPair[2]
				if err := invertHandlerFunc(in, blockX, blockY, p.Size, dcQT, chromaQT, setRowCb, predictCbPlaneDC); err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
			}
		}
	...
	for h := 0; h < dy/2; h += 16 {
		for w := 0; w < dx/2; w += 16 {
			...
			for _, p := range parts {
				...
				qtPair := qtBySize(p.Size)
				dcQT, chromaQT := qtPair[0], qtPair[2]
				if err := invertHandlerFunc(in, blockX, blockY, p.Size, dcQT, chromaQT, setRowCr, predictCrPlaneDC); err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
			}
		}
...
type predictDCFunc func(newImg *image.YCbCr, x, y int, size int) int16

func predictYPlaneDC(img *image.YCbCr, x, y, size int) int16 {
	return predictDC(img.Y, img.YStride, img.YOffset(x, y), x, y, size)
}

func predictCbPlaneDC(img *image.YCbCr, x, y, size int) int16 {
	return predictDC(img.Cb, img.CStride, img.COffset(x*2, y*2), x, y, size)
}

func predictCrPlaneDC(img *image.YCbCr, x, y, size int) int16 {
	return predictDC(img.Cr, img.CStride, img.COffset(x*2, y*2), x, y, size)
}

func predictDC(data []byte, stride int, offset, x, y int, size int) int16 {
	sum := 0
	count := 0
