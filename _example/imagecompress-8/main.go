...
func main() {
	...
	rowY := func(x, y int, size int, prediction int16) []int16 {
		...
		for i := 0; i < size; i += 1 {
			// - 128 = level shift
			plane[i] = int16(ycbcr.Y[ycbcr.YOffset(x+i, y)]) - prediction
		}
		return plane
	}
	rowCb := func(x, y int, size int, prediction int16) []int16 {
		...
		for i := 0; i < size; i += 1 {
			// 4:4:4 -> 4:2:0 への簡易間引き
			v := int16(ycbcr.Cb[ycbcr.COffset((x+i)*2, y*2)]) - prediction
			plane[i] = v
		}
		return plane
	}
	rowCr := func(x, y int, size int, prediction int16) []int16 {
		plane := make([]int16, size)
		for i := 0; i < size; i += 1 {
			v := int16(ycbcr.Cr[ycbcr.COffset((x+i)*2, y*2)]) - prediction
			plane[i] = v
		}
		return plane
	}
	...
	setRowCb := func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16) {
		...
		for i := 0; i < size; i += 1 {
			img.Cb[img.COffset((x+i)*2, y*2)] = clampU8(plane[i] + prediction)
		}
	}
	setRowCr := func(img *image.YCbCr, x, y int, size int, plane []int16, prediction int16) {
		...
		for i := 0; i < size; i += 1 {
			img.Cr[img.COffset((x+i)*2, y*2)] = clampU8(plane[i] + prediction)
		}
	}
	...
	transformHandlerFunc := func(w, h int, size int, dcQT, acQT QuantizationTable, rowN rowFunc, setRow setRowFunc, predict predictDCFunc) (*bytes.Buffer, error) {
	...
	for h := 0; h < dy; h += 32 {
		for w := 0; w < dx; w += 32 {
			...
			for _, p := range parts {
				...
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, lumaQT, rowY, setRowY, predictYPlaneDC)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
	...
	for h := 0; h < dy/2; h += 32 {
		for w := 0; w < dx/2; w += 32 {
			...
			for _, p := range parts {
				...
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, chromaQT, rowCb, setRowCb, predictCbPlaneDC)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
	...
	for h := 0; h < dy/2; h += 32 {
		for w := 0; w < dx/2; w += 32 {
			...
			for _, p := range parts {
				...
				qtPair := qtBySize(p.Size)
				dcQT, chromaQT := qtPair[0], qtPair[2]
				data, err := transformHandlerFunc(blockX, blockY, p.Size, dcQT, chromaQT, rowCr, setRowCr, predictCrPlaneDC)
				if err != nil {
					panic(fmt.Sprintf("%+v", err))
				}
...
func predictCbPlaneDC(img *image.YCbCr, x, y, size int) int16 {
	return predictDC(img.Cb, img.CStride, img.COffset(x, y), x, y, size)
}

func predictCrPlaneDC(img *image.YCbCr, x, y, size int) int16 {
	return predictDC(img.Cr, img.CStride, img.COffset(x, y), x, y, size)
}
